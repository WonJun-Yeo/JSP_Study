...
<%
	...
	// update는 executeUpdate 메소드를 사용하는데 Resulteset 객체가 필요한 이유가 있을까요?
			// DB에 해당 아이디가 없는 경우에 실행코드를 실행 시켜주기 위함
	// 폼에서 넘겨받은 id와 passwd를 DB에서 가져온 ID, PASSWD를 비교해 같을 경우 Update
	...
	try {
		...
		if(rs.next()) {
			...
			if (id.equals(rId) && passwd.equals(rPassword)) {
				// sql 변수 재사용
				sql = "UPDATE mbTbl SET name = '" + name + "', email = '" + email + "' WHERE id = '" + id + "' AND pass = '" + passwd +"'";
